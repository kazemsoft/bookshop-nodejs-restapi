...
router.post("/register", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "کاربر با موفقیت ثبت نام شد", user: newUser });
  } catch (error) {
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ message: "نام کاربری/رمز عبور نادرست وارد شده است" });
    }
    ...
    res.status(200).json({ message: "ورود با موفقیت انجام پذیرفت", token });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
